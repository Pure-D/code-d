...
export function downloadDub(env) {
	...
	if (process.platform == "linux" && process.arch == "x64") {
		url = "https://code.dlang.org/files/dub-1.3.0-linux-x86_64.tar.gz";
		ext = ".tar.gz";
	}
	else if (process.platform == "linux" && process.arch == "ia32") {
		url = "https://code.dlang.org/files/dub-1.3.0-linux-x86.tar.gz";
		ext = ".tar.gz";
	}
		...
		url = "https://code.dlang.org/files/dub-1.3.0-windows-x86.zip";
		ext = ".zip";
	}
	else if (process.platform == "darwin" && process.arch == "x64") {
		url = "https://code.dlang.org/files/dub-1.3.0-osx-x86_64.tar.gz";
		ext = ".tar.gz";
	}
	else
...
export function compileServeD(env) {
	var outputFolder = path.join(extensionContext.extensionPath, "bin");
	fs.exists(outputFolder, function (exists) {
		if (!exists)
			fs.mkdirSync(outputFolder);
		...
		compileDependency(outputFolder, "serve-d", "https://github.com/Pure-D/serve-d.git", [
			...
			vscode.window.showInformationMessage("serve-d successfully installed", "Reload").then((r) => {
				if (r == "Reload")
					vscode.commands.executeCommand("workbench.action.reloadWindow");
			});
		}, env);
