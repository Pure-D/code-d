...
export function searchDubPackages(query: string): Thenable<any[]> {
	return new Promise((resolve, reject) => {
		req()("https://code.dlang.org/api/packages/search?q=" + encodeURIComponent(query), function (error: any, response: any, body: any) {
...
export function listPackages(): Thenable<any[]> {
	return new Promise((resolve, reject) => {
		req()("https://code.dlang.org/packages/index.json", function (error: any, response: any, body: any) {
			if (error || response.statusCode != 200)
				return reject(error || "No packages found");
			var json: any[] = JSON.parse(body);
			resolve(json);
...
export function listPackageOptions(): Thenable<vscode.QuickPickItem[]> {
	...
	return new Promise((resolve, reject) => {
		req()("https://code.dlang.org/api/packages/search", function (error: any, response: any, body: any) {
			if (error || response.statusCode != 200)
				return reject(error || "No packages found");
...
export function getPackageInfo(pkg: string): Thenable<any> {
	return new Promise((resolve, reject) => {
		req()("https://code.dlang.org/api/packages/" + encodeURIComponent(pkg) + "/info", function (error: any, response: any, body: any) {
			if (error || response.statusCode != 200)
...
export function getLatestPackageInfo(pkg: string): Thenable<{ description?: string; version?: string; subPackages?: string[] }> {
	return new Promise((resolve, reject) => {
		req()("https://code.dlang.org/api/packages/" + encodeURIComponent(pkg) + "/latest/info", function (error: any, response: any, body: any) {
			if (error || response.statusCode != 200)
				return reject(error);
